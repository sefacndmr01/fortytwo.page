...
useSeoMeta({
  title: 'Data & Account Deletion Request — Fortytwo',
  description: 'Request deletion of your account or personal data from Fortytwo apps.',
...
const form = reactive({
  app: 'Chibi Studio',
  deletionType: 'both' as 'both' | 'account' | 'data',
  email: '',
  reason: '',
})
...
async function handleSubmit() {
  ...
  try {
    await $fetch('/api/delete-request', {
      ...
      body: {
        app: form.app,
        deletionType: form.deletionType,
        email: form.email,
  ...
  <main class="wrap">
    <template v-if="!submitted">
      <div class="page-head">
        ...
        <p class="sub">
          Submit this form to request deletion of your account or personal data.
      ...
      <form class="form" novalidate @submit.prevent="handleSubmit">
        ...
        <!-- Deletion type -->
        <div class="field">
          <label class="label">What would you like to delete?</label>
          <div class="radios">
            <label
              v-for="(label, val) in typeLabels"
              :key="val"
              class="radio"
              :class="{ 'radio--on': form.deletionType === val }"
            >
              <input v-model="form.deletionType" type="radio" :value="val" />
    ...
    <div v-else class="success">
      ...
      <p>
        ...
        <strong>5 business days</strong> to confirm and process your
        {{ typeLabels[form.deletionType].toLowerCase() }} deletion
        for {{ form.app }}.
      </p>
